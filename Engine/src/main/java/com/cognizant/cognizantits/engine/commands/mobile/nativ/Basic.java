...
public class Basic extends MobileNativeCommand {
   ...
   /**
    ...
    @Action(object = ObjectType.BROWSER, desc = "Allow Audio Recording(android)", input = InputType.YES)
    public void allowAudioRecording() {
    ...
    public void allowAudio() {
        try {
        	String webContext = ((MobileDriver) Driver).getContext();
        	Set<String> contexts = ((MobileDriver) Driver).getContextHandles();
            ...
            ((MobileDriver) Driver).findElement(By.xpath(".//android.widget.Button[@text='Allow']")).click();
            Thread.sleep(2000);
            ((MobileDriver) Driver).switchTo().alert().accept();
            Report.updateTestLog(Action, "Audio Recording Allowed", Status.PASS);
            ((MobileDriver) Driver).context(webContext);
        } catch (Exception ex) {
            Report.updateTestLog(Action, ex.getMessage(), Status.DEBUG);
            Logger.getLogger(Basic.class.getName()).log(Level.SEVERE, null, ex);
        }
